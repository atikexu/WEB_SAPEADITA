...
$(function() {
    ...
    $('.onlyNumbers').bind('paste', function() {VAR_CONTEXT
    });
...
/**
 * Validar formato monto sin comas
 * @param {String}variable
 * @returns {Boolean}
 */
...
function formatoMontoConComas(monto, decimal) {
	if ($.isNumeric(monto)) {
		var textMonto = monto.toString();
		monto = textMonto.trim().replace(/\,/g,'');//Suprimimos las comas antes de parsear el monto
		if (monto.trim() == '') {
			...
			monto = parseFloat(monto).toFixed(decimal).toString();
			var text = monto.trim();
			if (fnValidarFormatoWithOutComma(text)) {
				var num = monto.trim().replace(/\,/g,'');
				if (!isNaN(num)) {
...
function getKeyUser() {
	return '/' + usuarioBean.idEncriptado;
}

function obtenerManual(url){
	window.open(VAR_CONTEXT + '/resources/manual_online/inicio.html', '_blank');
}
